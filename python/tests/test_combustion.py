...
class CombustionDeclarationTests(unittest.TestCase):
    ...
    def test_public_api_matches_progress_variable_sketch(self) -> None:
        ...
        self.assertEqual(
            value["coupling"]["reaction_rate_basis"], "volumetric_mass"
        )
    ...
    def test_reaction_rate_basis_is_explicit_and_guarded(self) -> None:
        policy = fno.Combustion.CouplingPolicy(
            reaction_rate_basis="specific"
        )
        self.assertEqual(policy.to_plan()["reaction_rate_basis"], "specific")
        with self.assertRaisesRegex(ValueError, "reaction_rate_basis"):
            fno.Combustion.CouplingPolicy(reaction_rate_basis="molar")
    ...
    def test_reaction_rate_dictionary_template_is_concrete(self) -> None:
        ...
        self.assertIn("@REACTION_RATE_BASIS@", template)
        self.assertIn("autoCreateReactionRate true", template)
    ...
    def test_combustion_dictionary_wires_two_sessions_in_order(self) -> None:
        ...
        self.assertIn("reactionRateBasis     volumetricMass", rendered)
    ...
    def test_equation_source_uses_openfoam_combustion_interface(self) -> None:
        ...
        self.assertIn("explicitSpecificSource", helper)
        self.assertIn("density * reactionRate", helper)
        probe = (
            root
            / "tools/openfoam/progressVariableSourceProbe"
            / "progressVariableSourceProbe.C"
        ).read_text(encoding="utf-8")
        self.assertIn("mesh.V()[celli] * density[celli] * specificRate[celli]", probe)
        gate = (
            root / "tools/openfoam/testProgressVariableCombustion.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"--ranks"', gate)
        self.assertIn('"--reaction-rate-basis"', gate)
    ...
    def test_specific_rate_dictionary_and_native_density_conversion(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            case = combustion_case(Path(directory))
            specific = reaction_rate()
            case._fields["omega_c"] = Field(
                name="omega_c",
                field_class="volScalarField",
                dimensions="[0 0 -1 0 0 0 0]",
                internal_value=0.0,
                boundary_names=(),
                path=case.case_dir / "0" / "omega_c",
            )
            declaration = fno.Combustion.ProgressVariable(
                reaction_rate=specific,
                manifold=manifold(),
            ...
            longship = fno.Longship(case=case, combustion=declaration)
            reaction, table = longship.closure_programs
            _, rendered = render_combustion_dictionary(
                longship,
                reaction,
                table,
                "unix:///tmp/reaction.sock",
        ...
        self.assertIn("reactionRateBasis     specific", rendered)
        self.assertIn("reactionRateDimensions [0 0 -1 0 0 0 0]", rendered)
        root = Path(__file__).resolve().parents[2]
        progress_model = (
            root
            / "src/foamnordic/openfoam/models/progressVariableFjord"
        ...
        reaction_model = (
            root
            / "src/foamnordic/openfoam/models/reactionRateFjord"
            / "reactionRateFjord.C"
        ).read_text(encoding="utf-8")
        self.assertIn("this->thermo().rho()", progress_model)
        self.assertIn("explicitSpecificSource", progress_model)
        self.assertIn("this->thermo().rho()", reaction_model)
        self.assertIn("explicitSpecificSource", reaction_model)
