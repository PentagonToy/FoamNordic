...
void progressVariableFjord<ReactionThermo>::validateContract() const {
    ...
    if (reactionRateBasis_ != "volumetricMass"
        ...
        throw std::invalid_argument(
            "progressVariableFjord reactionRateBasis must be "
            "'volumetricMass' or 'specific'.");
    }
    if (reactionRateDimensions_ != expectedDimensions) {
        throw std::invalid_argument(
            "progressVariableFjord reactionRateDimensions do not match "
            "reactionRateBasis.");
    }
...
bool progressVariableFjord<ReactionThermo>::read() {
    ...
    if (configuredProgress != progressField_
        ...
        throw std::invalid_argument(
            "progressVariableFjord field identity, basis, and dimensions are "
