...
void FieldContract::validate() const {
    if (name.empty()) {
        throw std::invalid_argument("Model field name must not be empty.");
    }
    if (fjord::element_size(element) == 0) {
        throw std::invalid_argument("Model field element type is unsupported.");
    }
    if (components == 0) {
        throw std::invalid_argument("Model field components must be positive.");
    }
}

void ProgramContract::validate() const {
    if (name.empty()) {
        throw std::invalid_argument("Model contract name must not be empty.");
    }
    if (inputs.empty() || outputs.empty()) {
        throw std::invalid_argument("Model contract requires inputs and outputs.");
    }
        ...
        for (const auto& field : *fields) {
            ...
            if (!names.insert(field.name).second) {
                throw std::invalid_argument(
                    "Model field names must be unique within each direction.");
            }
        }
    }
}

std::vector<std::uint64_t> EvaluateAllCells::prepare(
    const TensorMap&,
    TensorMap&,
...
void EvaluateAllCells::merge(
    TensorMap& predictions,
    TensorMap& outputs,
    const std::vector<std::uint64_t>& active_cells,
    std::uint64_t cell_count) const {
    if (active_cells.size() != cell_count) {
        throw std::logic_error("EvaluateAllCells requires every cell to be active.");
    }
    outputs = std::move(predictions);
}

CellInferenceExchange::CellInferenceExchange(ProgramContract contract) : contract_(std::move(contract)) {
    contract_.validate();
}

void CellInferenceExchange::begin(
    ...
    if (state_ != ExchangeState::waiting && state_ != ExchangeState::completed) {
        throw std::logic_error("Model exchange cannot begin from its current state.");
    }
...
const FieldContract& CellInferenceExchange::require_field(
    ...
    if (field == fields.end()) {
        throw std::invalid_argument(
            std::string("Unexpected model ") + direction + " field: " + name);
    }
    return *field;
}

void CellInferenceExchange::validate_tensor(
    ...
    if (tensor.element != field.element || tensor.time_index != exchange_index_
        ...
        throw std::invalid_argument("Model tensor metadata does not match its exchange.");
    }
    ...
    for (std::size_t index = 1; index < tensor.shape.size(); ++index) {
        if (tensor.shape[index] != 0
            ...
            throw std::overflow_error("Model tensor component count overflowed.");
        }
        components *= tensor.shape[index];
    }
    if (components != field.components) {
        throw std::invalid_argument("Model tensor component count is incorrect.");
    }
}

void CellInferenceExchange::require_state(ExchangeState expected, const char* operation) const {
    if (state_ != expected) {
        throw std::logic_error(std::string(operation) + " is invalid in the current model state.");
    }
}

void CellInferenceExchange::add_input(fjord::Tensor tensor) {
    ...
    if (!inputs_.emplace(tensor.name, std::move(tensor)).second) {
        throw std::invalid_argument("Model input field was supplied more than once.");
    }
}

void CellInferenceExchange::seal_inputs() {
    require_state(ExchangeState::collecting_inputs, "seal_inputs");
    if (inputs_.size() != contract_.inputs.size()) {
        throw std::logic_error("Model inputs are incomplete.");
    }
    state_ = ExchangeState::inputs_ready;
}

const std::vector<std::uint64_t>& CellInferenceExchange::prepare(const CellEvaluationPolicy& policy) {
    ...
    if (!std::is_sorted(active_cells_.begin(), active_cells_.end())
        ...
        throw std::runtime_error("Model bypass policy returned invalid active cells.");
    }
    state_ = ExchangeState::evaluating;
    return active_cells_;
}

void CellInferenceExchange::add_output(fjord::Tensor tensor) {
...
void CellInferenceExchange::seal_outputs(const CellEvaluationPolicy& policy) {
    ...
    if (outputs_.size() != contract_.outputs.size()) {
        throw std::logic_error("Model outputs are incomplete after inference and bypass.");
    }
    for (const auto& field : contract_.outputs) {
        const auto output = outputs_.find(field.name);
        if (output == outputs_.end()) {
            throw std::logic_error("Model bypass did not produce every required output.");
        }
...
TensorMap CellInferenceExchange::finish() {
...
void CellInferenceExchange::fail(std::string reason) {
    if (reason.empty()) {
        reason = "Unknown model failure.";
    }
...
ExchangeState CellInferenceExchange::state() const noexcept { return state_; }
std::uint64_t CellInferenceExchange::exchange_index() const noexcept { return exchange_index_; }
double CellInferenceExchange::physical_time() const noexcept { return physical_time_; }
std::uint64_t CellInferenceExchange::solver_time_index() const noexcept {
    return solver_time_index_;
}
std::uint64_t CellInferenceExchange::cell_count() const noexcept { return cell_count_; }
const TensorMap& CellInferenceExchange::inputs() const noexcept { return inputs_; }
const std::string& CellInferenceExchange::failure_reason() const noexcept { return failure_reason_; }
