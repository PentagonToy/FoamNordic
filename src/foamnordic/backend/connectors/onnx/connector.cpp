...
public:
    ...
    bool supports(closure::ModelFormat format) const noexcept override {
        return format == closure::ModelFormat::onnx;
    }

    std::unique_ptr<closure::PackedModelKernel> load(
        const std::filesystem::path& payload,
        const closure::ModelArtifact&) const override {
        return std::make_unique<closure::OnnxPackedKernel>(payload);
    }
};
